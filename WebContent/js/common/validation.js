...
// if(/^[a-z][0-9]{3}$/.test('a001')){}  =  추출 가능한 지 검증.
...
String.prototype.validationBIR = function () {
    ...
    // 19990101
    // 990101
    return /^(19\d{2}|20\d{2})-(0\d|1[0-2])-(0\d|[1-2]\d|3[0-1])$/.test(this);
};
...
String.prototype.validationREGNO = function () {
    ...
    const moto = this.replace('-', '');
    const magicNum = this.substr(12, 1);
    let sum = 0;
    const checkSum = '234567892345';
    for(var i = 0; i < 12; i++) {
        sum += moto.charAt(i) * checkSum.charAt(i);
    }

    const finalValue = (11 - (sum % 11)) % 10;
